...
public class JoinOptimizer {
	 ...
	 * @param t1pkey Is the left-hand table a primary-key table? got it!!!!
	 ...
	 * @return A Vector<LogicalJoinNode> that stores joins in the left-deep order in
	...
	public Vector<LogicalJoinNode> orderJoins(HashMap<String, TableStats> stats,
		...
		Set<LogicalJoinNode> joinSet = new HashSet<>();
		...
		for (int i = 1; i <= size; i++) {
			...
			for (Set<LogicalJoinNode> s : subJoinSets) {
				...
				for(LogicalJoinNode node: s) {
					CostCard cc = computeCostAndCardOfSubplan(stats, filterSelectivities, node, s, bestCostSoFar, pc);
					if (cc != null && cc.cost < bestCostSoFar) {
						bestCostSoFar = cc.cost;
						bestCardSoFar = cc.card;
						bestOrder = cc.plan;
						
					}
				}
				pc.addPlan(s, bestCostSoFar, bestCardSoFar, bestOrder);
			}
		}
		// printJoins(joins, pc, stats, filterSelectivities);
		if (explain) {
			System.out.println("Optimal plan : ");
			for (LogicalJoinNode e : pc.getOrder(joinSet))
				System.out.print(e + " ");
			System.out.print("\n");
		}
	...
	public <T> HashMap<T, Set<T>> SubSets(Set<T> s) {
		HashMap<T, Set<T>> els = new HashMap<>();
